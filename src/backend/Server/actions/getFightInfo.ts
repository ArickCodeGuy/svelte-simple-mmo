...
export const useGetFightInfo =
  ...
  (livingId: number): FightInfo => {
    const living = serverController.livingsController.getById(livingId);
    if (!living || !living.fightInstanceId) return;
    ...
    const teams = {
      ally: instance[allyTeam].map((i) =>
        serverController.livingsController.getById(i.id)
      ),
      enemy: instance[enemyTeam].map((i) =>
        serverController.livingsController.getById(i.id)
      ),
    };
